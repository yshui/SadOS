 ...
 *    University, unless the copyrighter holder grant the submitter
 *    this permission..
 */
#include <tarfs.h>
...
{
    int i;
    struct dentry_reader* dentry_reader = malloc(sizeof(dentry_reader) ); 
    for (i = 0;i < tar_file_count;++i)
...
void tarfs_init()
{
    ...
    char *tarfs_start = res.buf;
    int tarfs_len = res.len;
    char *tarfs_end = tarfs_start + tarfs_len;
    ...
    struct posix_header_ustar *header = (struct posix_header_ustar*) (tarfs_start);

    while (header != (struct posix_header_ustar*)(tarfs_end) )
    {
